...
pub struct LocalSupportedFeatures {
    ...
    pub supported_features: LmpFeatureMask,
}

fn to_supported_features(bytes: &[u8]) -> Result<LocalSupportedFeatures, crate::event::Error> {
    require_len!(bytes, 9);
    Ok(LocalSupportedFeatures {
        supported_features: LmpFeatureMask::from_hci_bytes_complete(&bytes[1..])
            .map_err(crate::event::Error::ParseError)?,
    })
}
...
pub struct LeSupportedFeatures {
    ...
    pub supported_features: LeFeatureMask,
}
...
fn to_le_local_supported_features(
    ...
    Ok(LeSupportedFeatures {
        supported_features: LeFeatureMask::from_hci_bytes_complete(&bytes[1..])
            .map_err(crate::event::Error::ParseError)?,
    })
}
